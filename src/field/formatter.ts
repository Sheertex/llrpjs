  ...
  (base: AnyConstructor<LLRPFieldDescriptor, typeof LLRPFieldDescriptor>) =>
    class LLRPFormatterParser extends base {
      ...
      protected static parseIso8601Microseconds(timestamp: LLRPFMTDate): bigint {
        // seconds
        if (timestamp instanceof Date) timestamp = timestamp.toJSON()
        let time = Math.floor(Date.parse(timestamp) / 1000)

        let microseconds = parseInt(timestamp.match(ISO8601_REGEX_MICRO)?.toString() || '', 10)
        if (isNaN(microseconds)) microseconds = parseInt(timestamp.match(ISO8601_REGEX_MILLI)?.toString() || '', 10) * 1000

        if (isNaN(microseconds)) throw new LLRPError('ERR_LLRP_BAD_FORMAT', `cannot parse microseconds ${timestamp}`)
      ...
      protected static formatDateTime(value: bigint): string {
        return LLRPFormatterParser.fullPrecision ? LLRPFormatterParser.formatIso8601Microseconds(value) : new Date(Number(value) / 1000).toJSON()
      }
      ...
      protected static parseHexVector(v: string, bitWidth: number): number[] {
        ...
        v = v
          .trim()
          .split('')
          .reduce((acc, x, i) => (i % n == 0 && i != 0 ? `${acc},${x}` : `${acc}${x}`), '')
        return v.length ? v.split(',').map((x) => parseInt(x.padEnd(2, '0'), 16)) : []
      }

      protected static parseDateTime(v: LLRPFMTDate): bigint {
        return LLRPFormatterParser.fullPrecision ? LLRPFormatterParser.parseIso8601Microseconds(v) : BigInt(new Date(v).getTime()) * 1000n
      }
      ...
      public getFormatted(v: this['RV']) {
        let res: this['FMTV']
        if (this.isNormalFormattable) res = LLRPFormatterParser.formatNormal(v) // void
        else if (this.isDecFormattable) res = this.isVectorType ? LLRPFormatterParser.formatDecVector(v as number[] | bigint[]) : LLRPFormatterParser.formatDec(v as number | bigint)
        else if (this.isHexFormattable) res = this.isVectorType ? LLRPFormatterParser.formatHexVector(v as (number | bigint)[], this.bitWidth) : LLRPFormatterParser.formatHex(v as number | bigint, this.bitWidth)
        else if (this.isUtf8Formattable) res = LLRPFormatterParser.formatUTF8(v as string)
        else if (this.isDateFormattable) res = LLRPFormatterParser.formatDateTime(v as bigint)
      ...
      public getParsed(v: this['FMTV']) {
        let res: this['RV']
        if (this.isNormalFormattable) res = LLRPFormatterParser.parseNormal(v)
        else if (this.isDecFormattable) res = this.isVectorType ? LLRPFormatterParser.parseDec(v as string) : LLRPFormatterParser.parseDecVector(v as string)
        else if (this.isHexFormattable) res = this.isVectorType ? LLRPFormatterParser.parseHexVector(v as string, this.bitWidth) : LLRPFormatterParser.parseHex(v as string)
        else if (this.isUtf8Formattable) res = LLRPFormatterParser.parseUTF8(v as string)
        else if (this.isDateFormattable) res = LLRPFormatterParser.parseDateTime(v as LLRPFMTDate)
