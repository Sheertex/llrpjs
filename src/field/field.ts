...
export class LLRPField<FD extends FieldDescriptor> extends MixinAny(
  ...
  (base: ClassUnion<typeof LLRPFieldData, typeof LLRPFormatterParser, typeof LLRPEnumerator, typeof LLRPNode>) =>
    class LLRPField extends base {
      ...
      private isEVFormattable(v: this['EV']) {
        ...
        if (this.isBigInt) return this.isDateFormattable && (typeof v === 'string' || <any>v instanceof Date)
        return typeof v === 'string' && !this.isNormalFormattable
      }

      private isEVEnumerable(v: this['EV']) {
        if (!this.isUnsigned) return false
        if (Array.isArray(v)) return v.every((x) => typeof x === 'string') && this.isEnumerable
