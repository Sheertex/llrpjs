...
import { LLRPAllTypeDefinitions, LLRPMessageNames, GetDataType, GetDefFromRef, GetSubTypeRefs, LLRPParamNames, GetDataTypeFromFD, LLRPUserData, GetParamClassType, LLRPMessageI, GetTypedMessageCtrArgs, GetMessageClassType, LLRPDataValue } from '../types'
...
> extends LLRPProxyMessage<DT, N> {
  ...
  static ofType<AD extends LLRPAllTypeDefinitions, N extends LLRPMessageNames<AD>, TD extends AD[N] = AD[N], DT extends GetDataType<AD, TD> = GetDataType<AD, TD>>(Def: AD, Name?: N) {
    ...
    return (<unknown>_LLRPTypedMessage) as GetMessageClassType<AnyConstructor<LLRPTypedMessage<AD, N, TD, DT>>, AnyConstructor<LLRPProxyParameter>, AD, N, TD, DT>
  }
  ...
  setField<FN extends TD['fieldDescriptors'][number]['name']>(name: FN, v: LLRPDataValue) {
  ...
  addSubParameter<subName extends RefNames, ParamClass extends GetParamClassType<AnyConstructor<LLRPProxyParameter<LLRPUserData>>, AD, subName>>(name: subName, p: InstanceType<ParamClass>) {
    this.origin.addSubElement(name, p.origin)
    return this
  }

  setSubParameter<subName extends RefNames, ParamClass extends GetParamClassType<AnyConstructor<LLRPProxyParameter<LLRPUserData>>, AD, subName>>(name: subName, p: InstanceType<ParamClass>) {
    this.origin.setSubElement(name, p.origin)
    return this
  }

  getSubParameter<subName extends RefNames, ParamClass extends GetParamClassType<AnyConstructor<LLRPProxyParameter<LLRPUserData>>, AD, subName>>(name: subName) {
