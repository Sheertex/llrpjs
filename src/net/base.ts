...
export const LLRPNetNativeEvents = ['connection', 'close', 'connect', 'disconnect', 'message', 'error'] as const
...
export class LLRPNet {
  ...
  private async validRecv() {
    return new Promise<LLRPMessage>((resolve) =>
      this._ee.once('message', (msg) => {
        resolve(msg)
      }),
    )
  }
  ...
  private async onData(data: Buffer) {
    try {
      ...
      if (e instanceof LLRPError) this._ee.emit('error', e)
      else throw e
    }
  }
