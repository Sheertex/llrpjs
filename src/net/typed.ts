...
export class LLRPTypedNet<AD extends LLRPAllTypeDefinitions, N extends LLRPMessageNames<AD> = LLRPMessageNames<AD>, L extends LLRPNetNativeEvents = LLRPNetNativeEvents, MD extends GetAllTypedMessageClasses<AnyConstructor<LLRPTypedMessage<AD, N>>, AnyConstructor<LLRPProxyParameter>, AD> = GetAllTypedMessageClasses<AnyConstructor<LLRPTypedMessage<AD, N>>, AnyConstructor<LLRPProxyParameter>, AD>> extends LLRPNet {
  Def: AD
  ...
  private getNewListener<E extends N | L>(event: E, listener: AnyFunction) {
    const newListener = (m) => {
      if (LLRPNetNativeEvents.includes(<LLRPNetNativeEvents>event)) {
        ...
        } else newM = new LLRPTypedMessage((<LLRPProxyMessage<LLRPUserData, N>>m).origin as any)
        listener(newM)
      }
    }
  ...
  off<E extends N | L>(event: E, listener: AnyFunction) {
    ...
    if (newListener) this._ee.off(event, newListener)
  ...
  removeListener<E extends N | L>(event: E, listener: AnyFunction) {
    ...
    if (!pair) return this
    const [e, newListener] = pair
    if (newListener) this._ee.removeListener(event, newListener)
    this.listenerMap.delete(listener)
    return this
  }
  ...
  removeAllListeners(event?: N | L) {
    for (let pair of this.listenerMap) {
      ...
      if (!event || event === e) this.removeListener(e as any, listener)
    }
    return this
  }
}
