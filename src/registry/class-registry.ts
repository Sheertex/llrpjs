...
export class LLRPClassRegistry<AD extends LLRPAllTypeDefinitions, MN extends LLRPMessageNames<AD> = LLRPMessageNames<AD>, PN extends LLRPParamNames<AD> = LLRPParamNames<AD>> {
  ...
  static getInstance<AD extends LLRPAllTypeDefinitions, MN extends LLRPMessageNames<AD> = LLRPMessageNames<AD>, PN extends LLRPParamNames<AD> = LLRPParamNames<AD>>(def?: AD, name?: MN | PN) {
    if (!this.instance) {
  ...
  build() {
    for (let name in this.CoreDef) {
      ...
      if (td.isMessage) this.CoreMessageClasses[name as any] = LLRPTypedMessage.ofType(this.CoreDef, name)
      else if (!td.isMessage && td.typeNum >= 0) this.CoreParamClasses[name as any] = LLRPTypedParameter.ofType(this.CoreDef, name)
    }
    return this
  }
